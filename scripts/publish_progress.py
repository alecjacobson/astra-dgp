...
while not (P/'.stop-publisher').exists():
 if time.time()-start>=600:
  ...
  if candidates:
   ...
   subprocess.run(['git','add','progress'],cwd=P,check=True);subprocess.run(['git','commit','-m',f'Progress render {stamp}'],cwd=P,check=True);subprocess.run(['git','push'],cwd=P,check=True);print('PUBLISHED',dest,flush=True);start=time.time()
 time.sleep(5)
