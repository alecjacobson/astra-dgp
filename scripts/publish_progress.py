...
P=Path(__file__).resolve().parents[1]; seen=set(); start=time.time(); session_start=start
while not (P/'.stop-publisher').exists():
 if time.time()-start>=600:
  candidates=sorted((P/'renders').glob('*.png'),key=lambda p:p.stat().st_mtime)
  candidates=[p for p in candidates if p.stat().st_mtime>=session_start and (str(p),p.stat().st_mtime) not in seen and time.time()-p.stat().st_mtime>3]
  if candidates:
   ...
   for attempt in range(6):
    if subprocess.run(['git','-c','credential.helper=','-c','credential.helper=!gh auth git-credential','push','https://github.com/alecjacobson/astra-dgp.git','HEAD:main'],cwd=P).returncode==0:break
    time.sleep(5)
